...
export default function LikedAssets() {
  ...
  const getImageAssets = async (page) => {
    const limit = 20;
    const response = await assetService.getLikedAssets({ assetType: "image", page: page, limit })
  ...
  const getVideoAssets = async (page) => {
    const limit = 8;
    const response = await assetService.getLikedAssets({ assetType: "video", page: page, limit })
    if (response.status < 400 && response.data) {
  ...
  const getAudioAssets = async (page) => {
    const limit = 20;
    const response = await assetService.getLikedAssets({ assetType: "audio", page: page, limit })
    if (response.status < 400 && response.data) {
